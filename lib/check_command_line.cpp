...
void check_command_line_bilimbi(CalcOptions *CO, int argc, char *argv[]){
    ...
    bool computeset=false;
    CO->reference_origin = true; // default
    CO->periodic = false; // default

    for (int i = 1; i < argc; i++){
        std::string argvi=argv[i];
        if (argvi=="-i" || argvi=="--input")
        {
            if (i+1<argc && CO->infilename.size() == 0)
                CO->infilename=argv[++i];
            else {
    ...
    if (CO->outfoldername.size()==0){
        if (CO->infilename.size()>4 && CO->infilename.substr(CO->infilename.size()-4)==".bin")
            CO->outfoldername=CO->infilename.substr(0,CO->infilename.size()-4) + "_mink_val";
        else {
            std::cout << "no outfoldername can be defined" <<std::endl;
            exit(-1);
        }
